...
public class KVPair implements Comparable<KVPair> {
  ...
  /**
   * Constructor
   * @param k the key (first MemHandle)
   * @param v the value (second MemHandle)
   */
  ...
  /**
   * Compare a KVPair to a MemHandle, by comparing theKey to the
   * MemHandle's position.
   * Note that this relies on MemHandle having a compareTo method.
   * @return the ususal for a comparable (+, 0, -)
   * @param it the MemHandle to compare "this" against
   */
