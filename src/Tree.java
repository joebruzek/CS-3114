...
public class Tree {
		...
		if (node.getChild(0).isLeaf()) {
			...
			if (k.key().compareTo(node.getKeyV(0).key()) < 0) {
				this.add(node.getChild(0), k);
					...
					|| k.key().compareTo(node.getKeyV(1).key()) < 0) {
	...
	public void delete(TTNode node, KVPair k) {
		...
		if (node.isLeaf()) {
			if (node.getKeyV(0).compareTo(k) == 0 && node.getKeyV(1) != null) {
				...
				else if (((LNode) node).previous() != null
		...
		if (node.numRecs() == 2) {
			...
			if (node.getChild(0).isLeaf()) {
				if (node.getChild(0).getKeyV(0).compareTo(k) == 0) {
					...
					((LNode) node.getChild(1)).setPrevious(((LNode) node
							.getChild(0)).previous());
					...
					((INode) node).setKey(0, node.getChild(2).getKeyV(0));
					((INode) node).setKey(1, null);
					((LNode) node.getChild(2)).setPrevious(((LNode) node
							.getChild(1)).previous());
					...
					((INode) node).setKey(1, null);
					((LNode) node.getChild(1)).setNext(((LNode) node
							.getChild(2)).next());
			...
			if (node.getChild(0).getKeyV(0).compareTo(k) == 0) {
				...
				changeINodes(root, node.getChild(0).getKeyV(0), node
						.getChild(1).getKeyV(0));
				node.setChild(0, node.getChild(1));
				merge(root, node);
				...
				changeINodes(root, node.getChild(1).getKeyV(0), node
						.getChild(0).getKeyV(0));
	 ...
	 * 
	 * @param node the node
	 * @param k the KVPair to make proper
	 */
